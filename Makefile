...
kube-create-cluster:
	kops create cluster --state=s3://$(shell cd bootstrap && terraform output kops_state_bucket_name | xargs) --name=rmit.k8s.local --zones="us-east-1a,us-east-1b" --master-size=t2.small --node-size=t2.small --node-count=1 --yes

kube-secret:
	kops create secret --state=s3://$(shell cd bootstrap && terraform output kops_state_bucket_name | xargs) --name rmit.k8s.local sshpublickey admin -i ~/.ssh/id_rsa.pub
...
kube-delete-cluster:
	...
	kops delete cluster --state=s3://$(shell cd bootstrap && terraform output kops_state_bucket_name | xargs) rmit.k8s.local --yes
